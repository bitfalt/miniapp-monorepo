...
export default function NotificationDialog({ errorCode, onProceed, onDecline }: NotificationDialogProps) {
      ...
      >
        {errorMessage ? (
          <>
            ...
            <p className="mb-5 text-sm text-white sm:mb-6 sm:text-base md:text-lg">{errorMessage}</p>
            <button
          ...
          <>
            <h2 className="mb-2 text-lg font-semibold text-[#E36C59] sm:text-xl md:text-2xl">Before you continue:</h2>
            ...
            <p className="mb-5 text-sm text-white/90 sm:mb-6 sm:text-base md:mb-7 md:text-lg">
              By clicking "Yes, Proceed" you agree to receive notifications about new test releases, reminders to check
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              {["Yes, Proceed", "No, Maybe Later"].map((text, index) => (
                <button
                  key={text}
                  onClick={index === 0 ? handleProceed : handleDecline}
